...
namespace ge {
    ...
    struct InnerProductFn {
        ...
        template <std::weakly_incrementable LeftIter, std::sentinel_for<LeftIter> LeftSent, std::weakly_incrementable RightIter,
            ...
            requires Convertible<LeftIter, LeftProj, Init> && Convertible<RightIter, RightProj, Init> && requires (Init a) { a + a; a * a; }
        constexpr Init operator()(LeftIter leftIter, LeftSent leftSent, RightIter rightIter, Init init, LeftPred leftPred, RightPred rightPred, LeftProj leftProj = {}, RightProj rightProj = {}) const noexcept {
            for (; leftIter != leftSent; ++leftIter, ++rightIter)
                init = std::invoke(leftPred, std::move(init), std::invoke(rightPred, std::invoke(leftProj, *leftIter), std::invoke(rightProj, *rightIter)));
            return init;
        }

        template <std::ranges::input_range LeftRange, std::ranges::input_range RightRange, typename Init, typename RightProj = std::identity, typename LeftProj = std::identity>
            requires Convertible<std::ranges::iterator_t<LeftRange>, LeftProj, Init> && Convertible<std::ranges::iterator_t<RightRange>, RightProj, Init> && requires (Init a) { a + a; a * a; }
        constexpr Init operator()(LeftRange&& leftRange, RightRange&& rightRange, Init init, LeftProj leftProj = {}, RightProj rightProj = {}) const noexcept {
            return (*this)(std::ranges::begin(leftRange), std::ranges::end(leftRange), std::ranges::begin(rightRange), std::move(init), std::move(leftProj), std::move(rightProj));
        }

        template <std::ranges::input_range LeftRange, std::ranges::input_range RightRange, typename Init, typename LeftProj = std::identity, typename RightProj = std::identity,
                std::indirect_binary_predicate<std::projected<std::ranges::iterator_t<LeftRange>, LeftProj>, std::projected<std::ranges::iterator_t<RightRange>, RightProj>> RightPred,
                std::indirect_binary_predicate<Init, decltype(RightPred{}())> LeftPred>
            requires Convertible<std::ranges::iterator_t<LeftRange>, LeftProj, Init> && Convertible<std::ranges::iterator_t<RightRange>, RightProj, Init> && requires (Init a) { a + a; a * a; }
        constexpr Init operator()(LeftRange&& leftRange, RightRange&& rightRange, Init init, LeftPred leftPred, RightPred rightPred, LeftProj leftProj = {}, RightProj rightProj = {}) const noexcept {
            return (*this)(std::ranges::begin(leftRange), std::ranges::end(leftRange), std::ranges::begin(rightRange), std::move(init), std::move(leftPred), std::move(rightPred), std::move(leftProj), std::move(rightProj));
        }
