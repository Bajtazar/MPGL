...
namespace ge {
    ...
    struct AccumulateFn {
        ...
        template <std::input_iterator Iter, std::sentinel_for<Iter> Sent, Addable Init, typename Proj = std::identity>
            requires std::convertible_to<std::invoke_result_t<Proj, std::iter_value_t<Iter>>, Init>
        ...
        template <std::input_iterator Iter, std::sentinel_for<Iter> Sent, typename Init, typename Proj = std::identity,
        ...
        template <std::ranges::input_range Range, Addable Init, typename Proj = std::identity>
            requires std::convertible_to<std::invoke_result_t<Proj, std::ranges::range_value_t<Range>>, Init>
