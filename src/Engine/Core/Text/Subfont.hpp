...
namespace ge {
    ...
    private:
        ...
        Vector2ui getDimmensions(GlyphData const& glyph, std::size_t size) const noexcept;

        Vector2i getBearings(GlyphData const& glyph, std::size_t size) const noexcept;

        TextureVar renderTexture(Iter const& iter, std::size_t size) const;
