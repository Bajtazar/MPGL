...
namespace ge {
    ...
    Points::Points(const Vector2i& scene, std::size_t vertices, const Color& color) noexcept : Shape{scene, vertices} {
        for (auto& color_ : this->vertices | views::color)
            color_ = color;
    }

    Points::Points(const Vector2i& scene, std::size_t vertices) noexcept : Shape{scene, vertices} {}
