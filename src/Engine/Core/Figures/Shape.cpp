...
namespace ge {
    ...
    Shape::Shape(std::vector<Vertex>&& vertices) noexcept : vertices{std::move(vertices)} {}
    ...
    void Shape::onScreenTransformation(const Vector2ui& oldDimmensions) noexcept {
        for (auto& vertexPosition : vertices | std::views::transform(&Shape::Vertex::position)) {
            ...
            position = (position + 1.f) * vectorCast<float>(oldDimmensions) / vectorCast<float>(context.windowDimmensions) - 1.f;
        }
        copyToGPU();
    }
